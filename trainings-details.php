...
<script>
    window.onload = function() {
        ...
        var datefrom = urlParams.get('datefrom'); // Get the value of the 'datefrom' parameter
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            ...
            data: {
                ...
                datefrom: datefrom,
                day: day,
                schedule_id: scheduleId  // Ensure the parameter name matches PHP $_GET['schedule_id']
            },
                ...
                response.empData.forEach(function(empData) {
                    ...
                    if (!displayedEmpnos[empno]) {
                        ...
                        var newRow = $('<tr data-datefrom="' + empData.datefrom + '">' +
                            ...
                            '<input type="time" name="startTime" class="rounded border time-input" placeholder="00:00" value="' + empData.starttime + '"> - ' +
                            '<input type="time" name="endTime" class="rounded border time-input" placeholder="00:00" value="' + empData.endtime + '">' +
                            '</td>' +
        ...
        $('#doneBtn').on('click', function(e) {
            ...
            table.rows().every(function() {
                ...
                if (!$row.hasClass('disabled')) { // Exclude disabled rows
                    ...
                    var starttime = $row.find('input').eq(0).val();
                    var endtime = $row.find('input').eq(1).val();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    Swal.fire({
                        ...
                        var returnViewBatchSchedule = 'view-batch-schedules.php?id=' + id + '&schedule_id=' + scheduleId;
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        console.error('AJAX Error 3:', error);
                    }
                });
            }
        });
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        console.error('AJAX Error 4:', error);
                    }
                });
            }
        });
