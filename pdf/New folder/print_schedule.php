...
if (($datefromto == '2021-04-03')  AND $mtimein != '' AND $atimeout != '' AND ($datefrom <= $mtimein AND $dateto >= $atimeout)){
	
	@$spcregday1 = floor((strtotime($atimeout) - strtotime($datefrom))/3600) - $break;

}elseif (($datefromto == '2021-04-03') AND $mtimein != '' AND $atimeout != '' AND $datefrom >= $mtimein AND $dateto <= $atimeout){
...
if (($datefromto == '2021-04-03') AND $otimein != '' AND $otimeout != '' AND (date("Y-m-d", strtotime($otimeout)) == '2021-04-03')){
	
	$spcregday2 = floor((strtotime($otimeout) - strtotime($otimein))/3600);

}elseif(($datefromto == '2021-04-03') AND $otimein != '' AND (date("Y-m-d", strtotime($otimeout)) == '2021-04-04')){
 
	$spcregday2 = floor((strtotime('2021-04-04 00:00') - strtotime($dateto))/3600);
...
@$vldate = $row9['vldatefrom']; 
			...
			WHERE empno = '$empid' AND ottype = 'Overtime' AND otdatefrom = '$datefromto' AND otstatus = 'approved'";
			...
			<?php 
				...
				<td class="text-primary"><b>
					<center>
						...
						echo date('H:i', strtotime(@$row11['otfrom'])); 

						}elseif(@$row11['otstatus'] == 'pending' OR @$row11['otstatus'] == 'pending2'){
					...
					<center>
						...
						echo date('H:i', strtotime(@$row11['otto'])); 
...
if(@$_GET['backtrack'] != 'backtrack'){
						?>
				<tr>
					<td><center><b><?php echo $row3['COUNT(*)']; ?></b></center></td>
					<td><center><b><?php 
					if (@$totalssa != '') {
					echo @$totalssa; 
				}else{
					...
					<td><center><b><?php echo  (@$totalUT + @$totalUT2) - @$grandgen; ?></b></center></td>
				</tr>
...
$sqlo="SELECT * FROM generated
	...
	<tr>
					...
					<td><center><b>0</b></center></td>
					<td><center><b>0</b></center></td>
					<td colspan="2"><center><b>0</b></center></td>
					<td colspan="2"><center><b>0</b></center></td>
					<td><center><b>0</b></center></td>
					<td><center><b>0</b></center></td>
				</tr>
	...
	<tr>
					...
					<td colspan="2"><center><b><?php echo $rowo['nightdiff']; ?></b></center></td>
					<td colspan="2"><center><b><?php echo $rowo['vleave']; ?></b></center></td>
