...
<body id="page-top" class="sidebar-toggled">
    ...
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-2">
            <div class="mb-3">
                <h4 class="mb-0 mr-3 font-weight-bold">Service From Compassion</h4>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#generateResultButton').on('click', function() {
            ...
            // Extract and aggregate values for each service-from-compassion category
            tableData.forEach(function(row) {
                var evaluatedData = JSON.parse(row.responses);
                Object.keys(evaluatedData).forEach(function(category) {
                    if (category.startsWith('service-from-compassion')) {
    ...
    function redirectToHumilityList(id, responseName, evaluatedName, evaluatedPosition, evaluatedIdNumber) {
        ...
        window.location.href = `core-value-service-from-compassion-list.php?id=${id}&response_name=${encodedResponseName}&evaluated_name=${encodedEvaluatedName}&evaluated_position=${encodedEvaluatedPosition}&evaluated_idnumber=${encodedEvaluatedIdNumber}`;
}
